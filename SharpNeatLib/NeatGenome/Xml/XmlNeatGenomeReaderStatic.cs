...
namespace SharpNeatLib.NeatGenome.Xml
{
	public class XmlNeatGenomeReaderStatic
	{
        ...
        private static int[] ReadGrid(XmlNode xmlGrid)
        {
            if (xmlGrid == null)
                return null;

            List<int> ret = new List<int>();
            string[] s = XmlUtilities.GetAttributeValue(xmlGrid, "coords").Split(new char[] { ',' });
            foreach (string str in s)
            {
                if (str != "")
        ...
        private static List<int> ReadTrajectory(XmlNode xmlTrajectory)
        {
            if (xmlTrajectory == null)
                return null;

            List<int> ret = new List<int>();
            string[] s = XmlUtilities.GetAttributeValue(xmlTrajectory, "coordlist").Split(new char[] { ',' });
            foreach (string str in s)
            {
                if (str != "")
                    ret.Add(int.Parse(str));
