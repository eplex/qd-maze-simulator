...
namespace SharpNeatLib.NeatGenome
{
	...
	{	
		...
		public override INetwork Decode(IActivationFunction activationFn)
		{
			...
			{
				//network = GenomeDecoder.DecodeToConcurrentNetwork(this, ActivationFunctionFactory.GetActivationFunction("PlainSigmoid"));
...
//		}
        ...
        {
            Console.WriteLine("Wait, what? Why are we using modules (Mutate_AddModule())? This code is not maintained.");
...
//                  NeuronGene targetNeuron = neuronGeneList[tgtNeuronIdx];
                    ...
                    // JUSTIN: Check for feedforward connections only
                    if (ea.NeatParameters.feedForwardOnly)
                    {
                        if (sourceNeuron.Layer >= targetNeuron.Layer)
                        {
                            attempts--; // Do not count against the 5 attempts if this was an illegal connection
                            continue;
                        }
                    }
