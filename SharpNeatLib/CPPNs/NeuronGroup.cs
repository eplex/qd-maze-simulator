...
namespace SharpNeatLib.CPPNs
{
    public class NeuronGroup
    {
        ...
        public int Plane { get; set; } // Used for multi-plane substrates. The plane that this NG resides in.
        ...
        // Override that includes Plane identifier
        public NeuronGroup(float startX, float startY, float endX, float endY, uint dx, uint dy, int groupID, int groupType, uint globalID, int plane)
            : this(startX, startY, endX, endY, dx, dy, groupID, groupType, globalID)
        {
            Plane = plane;
        }
