...
namespace SharpNeatLib.CPPNs
{
    public class SubstrateDescription
    {
        ...
        public bool useLeo;
        public bool useMultiPlaneSubstrate; // NOTE: MPS is not compatable with LEO, neatbrain, multibrain, adaptable/modulatory networks, etc. It must be used alone, for now.
        public List<Point> planesConnected; // "unordered" pairs of planes that are connected. unordered by convention: largest plane id is always first. USE ACCESSOR METHODS (connectThesePlanes(), areThesePlanesConnected())
        public List<int> planes; // all plane ids present in the substrate. This is primarily used to count the number of planes.

        public bool areThesePlanesConnected(int first, int second)
        {
            if (first < second)
            { // If necessary, switch values so that "first" has the larger value
                int temp = first;
                first = second;
                second = temp;
            }
            return planesConnected.Contains(new Point(first, second));
        }
        private void connectThesePlanes(int first, int second)
        {
            if (first < second)
            { // If necessary, switch values so that "first" has the larger value
                int temp = first;
            ...
            planesConnected.Add(new Point(first, second));
        }
        private int indexOfPlaneConnection(int first, int second)
        {
            if (first < second)
            { // If necessary, switch values so that "first" has the larger value
                int temp = first;
            ...
            return planesConnected.IndexOf(new Point(first, second));
        }
        ...
        // MPS NOT supported by these methods. Also not checked for.. DO NOT USE MPS WITH THESE METHODS
